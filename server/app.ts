...
let server :Server = http.createServer((req: IncomingMessage, res: ServerResponse)=>{
   ...
   //get product by id
   else if(req.url?.match(/\/mercy\/api\/([0-9]+)/)  && req.method==="GET"){
   ...
   else if(req.url?.match(/\/mercy\/api\/([0-9]+)/)  && req.method==="PUT"){
...
else if(req.url?.match(/\/mercy\/api\/([0-9]+)/)  && req.method==="DELETE"){
